if (b4w.module_check("airplane"))
    throw "Failed to register module: airplane";
b4w.register("airplane", function(exports, require) {
...
exports.init= function(elapsed_sensor, is_char_htoh) {          
  ...
  for (var i = 0; i < m_conf.AIRPLANES_EMPTIES.length; i++) {  
		 empty_name=m_conf.AIRPLANES_EMPTIES[i];		
		var vehicle_empty = m_scs.get_object_by_name(empty_name);  
        var vehicle = m_scs.get_object_by_dupli_name(empty_name, "helicopter_collider");  
         _vehicle_wrapper = init_wrapper(vehicle, vehicle_empty, is_char_htoh); 		
		_is_char_htoh=_vehicle_wrapper.is_char_htoh;						
        m_ctl.create_sensor_manifold(_vehicle_wrapper, "AIRPLANE", m_ctl.CT_CONTINUOUS,     
...
function init_wrapper(body, empty, camera_target, is_char_htoh) {   
    return {
        body: body,    
        hp: m_conf.AIRPLANE_HP,    
        state: m_conf.AIRS_NONE,                      
       driving: null,    
        walk_speaker: null,
		run_speaker: null,
...
function vehicle_ai_cb(obj, id) {  
   if (obj.hp <= 0) {          
    ...
    switch (obj.state) {           
   case m_conf.AIRS_OVERFLY:
     	if (!m_sfx.is_playing(_vehicle_wrapper.run_speaker)) 
	{
	}
   case m_conf.AIRS_FLY:
        var elapsed = m_ctl.get_sensor_value(obj, id, 0);
        break;
		case m_conf.AIRS_IDLE:            
        var elapsed = m_ctl.get_sensor_value(obj, id, 0);
        break;
		case m_conf.AIRS_NONE:
        var elapsed = m_ctl.get_sensor_value(obj, id, 0);
        break;
   default:
        break;
...
/
 disable_object_controls = function(obj) {
  ...
  m_trans.set_translation_v(vehicle, m_conf.DEFAULT_POS_AIRPLANE);	 
