...
exports.setup_lava = function (elapsed_sensor) {  
    var time_in_lava = 0;
    function lava_cb(obj, id, pulse, elapsed) {  
        if (pulse == 1) {
            ...
            if (time_in_lava >= m_conf.LAVA_DAMAGE_INTERVAL) {   
				...
				{
					m_char.change_hp(-damage);
					console.log("se deberia haber producido damage en character de :"+damage);
				}
                time_in_lava = 0;
            }
			m_char.change_hp(-10);  
		...
		console.log("se deberia haber producido damage en character por tocar la lava un tiempo");
    }  
    var char_wrapper = m_char.get_wrapper();
    var lava_ray = m_ctl.create_ray_sensor(char_wrapper.picker, [0, 0, 0], [0, 0, -1],   
                                           "LAVA", true);
 m_ctl.create_sensor_manifold(char_wrapper.picker, "LAVA_COLLISION",        
       m_ctl.CT_TRIGGER, [lava_ray],
        function(s) {return s[0]}, lava_cb, elapsed_sensor);
}
